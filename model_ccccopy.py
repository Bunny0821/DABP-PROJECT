...
first_class_limit = total_orders/16
same_day_limit = total_orders/16*3
second_class_limit = total_orders/16*5
standard_class_limit = total_orders/16*7

m.addConstr(quicksum(x[i, 'First_Class'] for i in orders) <= first_class_limit)
m.addConstr(quicksum(x[i, 'Same_Day'] for i in orders) <= same_day_limit)
m.addConstr(quicksum(x[i, 'Second_Class'] for i in orders) <= second_class_limit)
m.addConstr(quicksum(x[i, 'Standard_Class'] for i in orders) <= standard_class_limit)
